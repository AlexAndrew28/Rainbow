...
from utils.PriorityExperianceBuffer import SimplePriorityExperienceBuffer
...
class DQN(nn.Module):
    def __init__(
            ...
            batch_size: int=64, 
            gamma: float=0.99, 
            priority_buffer_eps: float=0.01,
            priority_buffer_alpha: float= 0.5,
            priority_buffer_alpha_decay: float=0.99,
            beta: float=0.3,
        ...
        self.experiance_buffer = SimplePriorityExperienceBuffer(buffer_size, eps=priority_buffer_eps, alpha=priority_buffer_alpha, alpha_decay=priority_buffer_alpha_decay)
    ...
    def get_loss(self, target_model: nn.Module) -> torch.Tensor:
        ...
        indx, sample, priorities = self.experiance_buffer.getSample(amount=self.batch_size)
        ...
        new_priorities = loss.detach()

        # update priorities based on the loss
        self.expBuffer.updatePriorities(indx, new_priorities)
