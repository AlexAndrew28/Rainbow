...
class DuelingDQN(nn.Module):
    def __init__(
            ...
            output_size: int, 
            lr:float=0.0002, 
            number_of_hidden_layers_main_stream: int = 3,
            dense_layer_size: int=36, 
            number_of_hidden_layers_adv_stream: int = 1,
            number_of_hidden_layers_value_stream: int = 1,
        ...
        self.model_layers = []
        for i in range(number_of_hidden_layers_main_stream):
            self.model_layers.append(nn.Linear(dense_layer_size, dense_layer_size))
            self.model_layers.append(nn.ReLU())
            
        self.model_layers_advantage = []
        for i in range(number_of_hidden_layers_adv_stream):
            self.model_layers_advantage.append(nn.Linear(dense_layer_size, dense_layer_size))
            self.model_layers_advantage.append(nn.ReLU())
        self.model_layers_advantage.append(nn.Linear(dense_layer_size, output_size))

        self.model_layers_value = []
        for i in range(number_of_hidden_layers_value_stream):
            self.model_layers_value.append(nn.Linear(dense_layer_size, dense_layer_size))
            self.model_layers_value.append(nn.ReLU())
        self.model_layers_value.append(nn.Linear(dense_layer_size, 1))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for model_layer_adv in self.model_layers_advantage:
            adv = model_layer_adv(adv)
            
        for model_layer_value in self.model_layers_value:
            value = model_layer_value(value)
            
        y = value + adv - adv.mean(dim=-1, keepdim=True)
